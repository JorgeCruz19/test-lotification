...
const EditLotification = () => {
	...
	useEffect(async () => {
		const fields = ["name"];
		const docRef = doc(db, "prueba", `${idDepartment}`, "ciudades", `${idCity}`, "lotificaciones", `${idLotification}`);
	...
	const onSubmit = async (data) => {
		const { name } = data;
		setLoading(true);
		await updateDoc(doc(db, "prueba", `${idDepartment}`, "ciudades", `${idCity}`, "lotificaciones", `${idLotification}`), {
			name: name,
		});
		setLoading(false);
		toast.success("Se actualizó la lotificación correctamente");
	};

	return (
		<div className="w-full md:w-2/3 mx-auto">
			<h2 className="mb-8 font-bold text-2xl">Editar lotificación</h2>
...
export default EditLotification;
