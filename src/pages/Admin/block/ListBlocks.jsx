...
const ListBlocks = () => {
	...
	const { idDepartment, idCity, idLotification } = useParams();

	const handleDelete = async (id) => {
		try {
			await deleteDoc(doc(db, "prueba",`${idDepartment}`,"ciudades",`${idCity}`,"lotificaciones", `${idLotification}`, "blocks", id))
			const filterBlocks = lotifications.filter((item) => item.id !== id)
			setLotifications(filterBlocks);
			toast.success("Se elimino el bloque correctamente");
		} catch (error) {
			console.log(error);
		}
	};
		...
		<>
			{isLoading ? (
				...
				<div className="w-full md:w-2/3 mx-auto">
					<h2 className="mb-8 font-bold text-2xl">Lista de Bloques</h2>
					<div className="relative overflow-x-auto shadow-md sm:rounded-lg">
						<table className="w-full text-sm text-left text-gray-500 ">
							...
							{lotifications.length ? lotifications.map(({ id, data }) => (
								...
								:
								<tr>
									<td className="p-3 text-center" colSpan={3}>No tiene bloques</td>
								</tr>
								}
							</tbody>
						</table>
