...
const CreateDepartment = () => {
	...
	const onSubmit = async (data) => {
		...
		querySnapshot.forEach((doc) => {
			departmentsArr.push({ ...doc.data() });
		});
		const filterDepartment = departmentsArr.filter((department) => {
			return department.deparment.toLowerCase().includes(name.toLowerCase());
		});

		if (filterDepartment.length) {
			toast.error("Ya existe el departamento, intenta con otro nombre.");
		} else {
			await addDoc(collection(db, "prueba"), {
				deparment: name,
			});
			reset({ name: "" });
			toast.success("Se agregó el departamento correctamente");
		}
		setLoading(false);
	};
	...
	return (
		<>
			<div className="w-full md:w-2/3 mx-auto">
				<h2 className="mb-8 font-bold text-2xl">Crear Departamento</h2>
