...
const EditCity = () => {
	...
	const { idDepartment, city: cityParam } = useParams();
	const [loading, setLoading] = useState(false);

	useEffect(async () => {
		const fields = ["name"];
		const docRef = doc(db, "prueba", `${idDepartment}`, "ciudades", `${cityParam}`);
		const docSnap = await getDoc(docRef);
		const { city } = docSnap.data();
		fields.forEach((field) => setValue(field, city));
	}, []);

	const onSubmit = async (data) => {
		const { name } = data;
		setLoading(true);
		await updateDoc(doc(db, "prueba", `${idDepartment}`, "ciudades", `${cityParam}`), {
			city: name,
		});
		setLoading(false);
		toast.success("Se actualizó la ciudad correctamente");
	};

	return (
		<div className="w-full md:w-2/3 mx-auto">
			<h2 className="mb-8 font-bold text-2xl">Editar ciudad</h2>
