...
const CreateDepartment = () => {
	...
	useEffect(async () => {
		try {
			...
			setIsLoading(false);
			setDepartmentsSelect(departmentsArr);
			return () => {
				isMounted = false;
			};
		} catch (error) {
	...
	const onSubmit = async (data) => {
		const { name, departmentsSelect } = data;
		setIsLoading(true);
		await addDoc(collection(db, "prueba", `${departmentsSelect}`, "ciudades"), {
			city: name,
		});
		setIsLoading(false);
		reset({ name: "", departmentsSelect: "" });
		toast.success("Se agregó la ciudad correctamente");
	};

	return (
		<>
			<div className="w-full md:w-2/3 mx-auto">
				<h2 className="mb-8 font-bold text-2xl">Crear Ciudad</h2>
					...
					<div className="mb-6">
						...
						{errors.departmentsSelect && (
							<p className="mt-2 text-sm text-red-600">
								{errors.departmentsSelect.message}
							</p>
						)}
					</div>
